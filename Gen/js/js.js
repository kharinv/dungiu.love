...
function darkside(p1,tr){
	...
	if(p1!="" && p1!=null){
		...
		for(var k=1;k<=ccghm;k++){
			...
			var cn=chkCard(p);
			for(var i=tr;i>=1;i--){
				document.console.output2.value="Generator...";
			...
			}else var out="Sorry, no valid BINs are inserted or BIN is incomplete, please check that the format is correct or not: '666666xxxxxxxxxx' ";
		}
		if(jou==2) var out=out+"</xml>";
		else if(jou==3) var out=out+"}";
...
function cleanText(){
	var confirmar=confirm("Do you want to delete the created tags?");
